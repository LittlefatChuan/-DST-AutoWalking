...
local isOpenListEmpty = function(search)
	return #search.frontier == 0
end
...
local function SamplingCheckWalkable(pos, target_pos, pathcaps, sampling_dist)
	...
	local sampling_dist = sampling_dist or 2 -- half of TILE_SCALE
...
local function CheckWalkableFromPoint(pos, target_pos, pathcaps, without_check_potiental_walkable)
	local pathcaps = pathcaps or {ignorewalls = false,  ignorecreep = false, ignoreflood = false, ignoreLand = false, allowocean = false, }
	-- precheck
	...
	--------------------------------------------
	if not pathcaps.ignoreflood then
		local check_flood_pathcaps = {ignorewalls = true, ignorecreep = true, ignoreflood = false, allowocean = true}
...
local PATHCAPS_STR = {ignorewalls = "W", ignoreLand = "L", ignorecreep = "C", ignoreflood = "F", allowocean = "O"}
local function pathcapToStr(pathcaps)
	local pathcaps = pathcaps or {}
	local ret = {}
	for k, v in pairs(PATHCAPS_STR) do
		if pathcaps[k] then
			table.insert(ret, PATHCAPS_STR[k])
		end
	end
	return table.concat(ret)
end

-- avoid redundant calculations for the same points
local function CheckWalkableFromPointCached(search, fromPoint, toPoint, pathcaps)
	-- dont consider pathcaps since we always use same pathcaps in one search
    -- local key = fromPoint.x .. "_" .. fromPoint.z .. "_" .. toPoint.x .. "_" .. toPoint.z .. pathcapToStr(pathcaps)
	-- local key_rev = toPoint.x .. "_" .. toPoint.z .. fromPoint.x .. "_" .. fromPoint.z .. pathcapToStr(pathcaps)
    -- if search.walkableCache[key] == true or search.walkableCache[key_rev] == true then
    --     return search.walkableCache[key] or search.walkableCache[key_rev]
    -- else
    --     local result = CheckWalkableFromPoint(fromPoint, toPoint, pathcaps)
	-- 	if result == true then
    --     	search.walkableCache[key] = result
	-- 	end
    --     return result
    -- end
	return CheckWalkableFromPoint(fromPoint, toPoint, pathcaps)
end
...
local makePath = function(search, finalCoord)
	...
	if search.endPos ~= search.original_endPos and not CheckWalkableFromPointCached(search, search.endPos, search.original_endPos, search.pathcaps) then
	...
	if search.original_startPos ~= search.startPos and not CheckWalkableFromPointCached(search, search.original_startPos, search.startPos, search.pathcaps) then
...
local smoothPath = function(search)
	...
	while(index < #(path.steps)) do
		...
		local is_on_speedup_turf = calcGroundSpeedMulti(curPoint, search.groundcaps) == ASTAR_COSTMULTI_FASTER

        -- dont remove the points that on speedup_turf even if they're have LOS with previous point
		if CheckWalkableFromPointCached(search, prePoint, postPoint, check_pathcaps) and
...
local function is_point_without_wall(point)
	return not TheWorld.Pathfinder:HasWall(point.x, 0, point.z)
end

local function SearchNearbyPointWithoutWall(point, start_angle, radius, attempts)
	-- params:position, start_angle, radius, attempts, check_los, ignore_walls, customcheckfn, allow_water, allow_boats
	local offset = FindWalkableOffset(point, start_angle, radius, attempts, true, true, is_point_without_wall)
	return offset and (offset + point) or nil
end
...
local requestSearch = function(startPos, endPos, pathcaps, groundcaps)
	...
	if pathcaps == nil then
		...
		search.pathcaps = { player = true, ignorecreep = true, ignoreflood = true}
	else
	...
	-- else
		...
		search.walkableCache = {}		-- dict, store the walkable state of points pairs 
		
		search.path = nil
		pushIntoOpenList(search, makeCoord(0,0,0))
...
local processSearch = function(search, workPerRound, maxWork)
	...
	local currentCoord, currentPoint, candidatePoints, nextCoord, nextPoint
	-- Process until finished (no search.frontier remain or a path is found)
	while not isOpenListEmpty(search) do
		
		-- get the coord of min F value
		currentCoord = popFromOpenList(search)
		currentPoint = coordToPoint(origin, currentCoord)
		...
		if calcCost(currentPoint, search.endPos) <= PATH_NODE_DIST
				and CheckWalkableFromPointCached(search, currentPoint, search.endPos, search.pathcaps) then
			...
			for point=1,8,1 do
			--for point=1,4,1 do 
				nextCoord = candidatePoints[point]
				nextPoint = coordToPoint(origin, nextCoord)
			
				if CheckWalkableFromPointCached(search, currentPoint, nextPoint, search.pathcaps)
...
-- 					local is_on_faster_tiles = search.groundcaps.faster_on_tiles and search.groundcaps.faster_on_tiles[tostring(TheWorld.Map:GetTileAtPoint(nextPoint.x, 0, nextPoint.z))]
					...
					if not visited(search, nextCoord) or new_cost < search.g_score_so_far[nextCoord.x][nextCoord.y] then
