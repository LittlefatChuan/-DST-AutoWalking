...
-- ngl_pathfinder: 将游戏自带的寻路接口进行了封装，来统一两个寻路器的接口给pathfollower调用, 具体实现不知，听说是C++层的分层A星寻路
-- ngl_obstaclepfwall: 弃用，已经更换成ngl_pfwalls_clientonly
-- ngl_pfwalls_clientonly :仅在客户端上给更多的障碍物加上寻路墙，当寻路完成，人物跟随路径时，会通过发送事件来注册障碍物的寻路墙，同时用官方的寻路进行再次分段寻路，然后在拼接回原路径进行动态的路径调整
	...
	-- 服务器版本：见Don't blocked together 里components/ngl_pfwalls 和ngl_pfwalls_replica，在服务器和客户端上在游戏一开始就都添加了更多障碍物的寻路墙
-- 游戏自带寻路的一点线索（陈年老帖）：
