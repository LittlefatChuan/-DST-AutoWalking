local AStarUtil = require("utils/astar_util")
...
local Pathfinder = Class(function(self, inst)
	...
	self.process_work_per_round = 600 --step
	self.process_maxwork = 25000 --step
	self.process_maxtime = 1.2 --sec
end)
...
function Pathfinder:SubmitSearch(startPos, endPos, pathcaps, groundcaps)
	...
	self.search_process = self.inst:DoPeriodicTask(self.process_period,
						function()
							if AStarUtil.processSearch(self.search_data, self.process_work_per_round, self.process_maxwork) or -- pathsearch finished
									(self.process_maxtime and self.search_start_time and GetTime() > self.search_start_time + self.process_maxtime) then -- timeout
